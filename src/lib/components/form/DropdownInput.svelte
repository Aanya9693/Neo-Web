...
<label for="dropdown" class="block text-sm mb-2">
	{label}
</label>
...
<div class="selectdiv">
	<div
		id="dropdown"
		class={`custom-dropdown  focus:ring-2 focus:ring-primary text-gray-500 text-md py-3 w-full p-2.5 block relative ${roundedfull ? 'rounded-full border-none bg-[#112D44]': 'rounded-md border border-gray-600 bg-[#0E1F2D]'} `}
	>
		<!-- svelte-ignore a11y-click-events-have-key-events -->
		<div class={`selected-option ${bright ? 'text-[#06E7ED]':'text-white'}`} on:click={() => (isDropdownOpen = !isDropdownOpen)}>
			{selectedValue}
