...
<div class="w-full h-screen">
	<Modal>
		<div class="w-[718px] p-10 bg-[#112D44] rounded-xl">
			<div class="text-center mb-3">Receive Funds</div>
			<div class="mx-auto w-[368px] bg-[#081B2A] p-1 flex rounded-lg">
				...
				<span
					class={`cursor-pointer w-1/2 rounded-md py-3 text-md text-center ${
						activeTab === 2 ? 'bg-[#112D44] text-white' : 'text-[#92B0C999]'
					}`}
			...
			{#if activeTab === 0}
				<div class="flex flex-col gap-10 my-10 px-10">
					<div>
						<Input type="number" editIcon={false} label="Add amount" name="amount" value={0} />
					</div>
					<div>
						<DropdownInput
							label="UPI ID"
			...
			{:else if activeTab === 1}
				<div class="flex flex-col justify-center items-center my-6 gap-6 px-10 w-full">
					<p class="text-xs text-[#92B0C999]">Scan the QR code to add money</p>

					<img
						src="../../../dashboard/dashboard/Frame 35407.png"
						class="w-[200px]"
						alt=""
						srcset=""
					/>
					<CopyToClipboard content="UPI87676471okici7@Neucron.io" />
				</div>
				<div class="flex justify-center">
					<button
						class="text-[16px] font-bold bg-primary text-primary-dark hover:bg-white py-2 px-8 rounded"
			...
			{:else if activeTab === 2}
				<div class="flex flex-col gap-5 my-6 px-10 w-full">
					<div class="flex flex-col items-start gap-1">
						<h1 class="text-xl flex items-start gap-3">
							Virtual Account Details <Icon icon="icon-park-solid:down-one" color="#06e7ed" />
						</h1>
						<p class="text-xs text-[#92B0C999]">
							To add funds to your Neucron wallet, transfer funds via UPI/NEFT/RTGS/IMPS to the
							account below
						</p>
					</div>
